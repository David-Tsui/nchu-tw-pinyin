...
<html lang="en">
    <head>
        ...
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-origin.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-blue.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-pink.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-xmas.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-origin-flat.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-blue-flat.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-pink-flat.1.4.0.css') }}" rel="stylesheet">
        <link href="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-xmas-flat.1.4.0.css') }}" rel="stylesheet">
        ...
        <script>
            ...
            var mode = 0;                                                       // 自選模式 0: 拼音模式; 1: 選字模式; 2: 關聯詞模式  
            ...
            var three_pinyin = ["","",""];                                      // 陣列-記錄三詞分別的拼音
            var three_pinyin_loc = [0,999999,999999];                           // 陣列-記錄三詞拼音分別的起始位置
            var three_word_loc = [0,999999,999999];                             // 陣列-記錄三詞起始位置
            ...
            var tow_check = false;                                              // 判斷滑鼠有無反白拖曳(僅對選字跟拼音時做判斷)
            ...
            var email_flag = false;                                             // 判斷email是否正確的flag，因為ajax post的關係所以才要用
            ...
            $(document).ready(function(){
                ...
                $("#input").mousedown(function(){                           
                    ...
                    /*if (forbid_mousemove == false){
                        ...
                        if (show_text == ""){                                    // 非選字時點選textbox中的任一位置，或是拖拉反白
                            ...
                            if (getCaretCharacterOffsetWithin(DOM_textbox) == getLocation_End(textbox)){ // 如果在拼音或選字時，將游標點選之後依舊在原始位置
                                tow_check = false;
                            }
                            else                                                 // 如果在拼音或選字時反白
                                tow_check = true;
                    ...
                    if (tow_check == true && show_text != ""){                  // 如果不能選字，或是拼音還存在，則反白會失效，游標自動跑到當前的input_loc
                ...
                $("#input").keydown(function(e){
                    ...
                    input_copy = textbox.html();                                // 先備份鍵入前的textbox中之值，預留給換頁+/-號時使用
                    temp_loc = getCaretCharacterOffsetWithin(DOM_textbox);      // 暫存按下時的輸入位置，按下+/-號時還原此位置                
                    show_text = $("#show").html();
                    auto_start = get_auto_start();
                    
                    if (show_text == "" || show_text == "無此拼音"){             // 如果拼音模式下失敗，禁止+/-號
                    ...
                    if (keyin == 13 && sel_mode == 1){							// 智能模式的自動選詞時，按下enter結束自動選詞
                    	...
                    	if (mode == 2){
                    		for(var i = 0; i < three_word_loc.length; i++){
                    			if (three_word_loc[i] < 999999)
	                    			input_loc = three_word_loc[i] + auto_letter[i].length;                   		
                    		}                  		
                    ...
                    if (keyin == 35){                                          // 限制End鍵
                        ...
                        else if (mode == 1 || show_text == "無此拼音"){         // 選字模式下，若按下End鍵直接將輸入位置移動到拼音尾
                            textbox.setCursorPosition(input_loc + search_key.length);
                            return false;
                        }
                    ...
                    if (keyin == 36){                                          // 限制Home鍵
                        ...
                        else if (mode == 1 || show_text == "無此拼音"){         // 選字模式下，若按下Home鍵直接將輸入位置移動到拼音首
                            textbox.focus().setCursorPosition(input_loc);
                            return false;
                        }
                        if (mode == 2){                                        // 關聯詞模式下，若按下方向鍵視為放棄
                    ...
                    /*if (keyin == 40 && sel_mode == 0 && mode == 0){

                    }*/

                    if (keyin == 40 && sel_mode == 1 && mode == 2){            // 自動選詞時按方向鍵"下"來改字
                        if (temp_loc >= three_word_loc[0] && temp_loc < three_word_loc[1]){
                            var j = 0;
                            var pinyin = three_pinyin[0];
                        ...
                        else if (temp_loc >= three_word_loc[1] && temp_loc < three_word_loc[2]){
                            var j = 0;
                            var pinyin = three_pinyin[1];
                            var split_loc = (temp_loc - auto_start);
                            for(var i = 0; i < pinyin.length; i++){
                                if (pinyin[i] == " ")                           //
                                    j++;
                        ...
                        else if (temp_loc >= three_word_loc[2]){
                            var j = 0;
                            var pinyin = three_pinyin[2];
                            var split_loc = (temp_loc - auto_start);
                            for(var i = 0; i < pinyin.length; i++){
                                if (pinyin[i] == " ")                           //
                                    j++;
                    ...
                    if (keyin == 8 && reachLeft() == true && sel_mode == 1 && mode == 2) return false; 
                    if (keyin == 46 && reachRight() == true && sel_mode == 1 && mode == 2) return false; 
                    ...
                    if (!(keyin == 107 || keyin == 187 || keyin == 109 || keyin == 189 || (keyin >= 48 && keyin < 57) || (keyin >= 96 && keyin < 105) )){  
                        // 換頁時，按下+/-號、數字不影響輸入游標位置                   
                        getCaretCharacterOffsetWithin(DOM_textbox);
                    }               
                    ...
                    if (sel_mode == 0){ 
                        ...
                        if (keyin == 35 && associated_search_flag == false){
                            input_loc = getCaretCharacterOffsetWithin(DOM_textbox);  
                        }
                        if (keyin == 36 && associated_search_flag == false){
                            input_loc = getCaretCharacterOffsetWithin(DOM_textbox); 
                        }
                    }
                    if (sel_mode == 1){
                ...
                $.post('search_word_auto.php',{auto_KEY:auto_search_key},function(data){
                        if (data == ""){
                            ...
                            for(var i = 1; i <= len; i++){
                                ...
                                three_pinyin[j++] = data[i]['pinyin'];
                            }

                            if (check_over_three == 1){                 // 如果是，則調整auto_search_key
                                var cut_index = auto_search_key.indexOf(three_pinyin[0]);   // 第一個要固定詞的起始位置
                                var first_word_len = three_word_loc[1];
                                auto_start = first_word_len;			// 調整auto_start為當前第二詞的起始位置
                                var first_pinyin_len = three_pinyin[0].length;
            ...
            function setWord(){                                         // 把中文字打上去
                ...
                var prompt_txtbox = $("#prompt");
                var prompt_flat_txtbox = $("#prompt_flat");
                input_word = remove_tags(input_word);                   // 
                ...
                if (sel_mode == 0 || (sel_mode == 1 && mode == 3)){
                    if (mode == 3){
                        if (keyin == 48 || keyin == 96){
                            var word_length = select_letter[9 + (thispage - 1) * 10].length;
                            var replace_end = input_loc + select_letter[9 + (thispage - 1) * 10].length;
                            right = right.substring(replace_end,input_word.length);            
                            console.log("left: " + left);
                            console.log("right: " + right);              
                            input_word = left + select_letter[9 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                        }
                        else if (keyin >= 97){
                            var word_length = select_letter[keyin - 97 + (thispage - 1) * 10].length;
                            var replace_end = input_loc + select_letter[keyin - 97 + (thispage - 1) * 10].length;
                            right = right.substring(replace_end,input_word.length); 
                            console.log("left: " + left);
                            console.log("right: " + right);
                            input_word = left + select_letter[keyin - 97 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                        }
                        else if (keyin >= 49){
                            var word_length = select_letter[keyin - 49 + (thispage - 1) * 10].length;
                            var replace_end = input_loc + select_letter[keyin - 49 + (thispage - 1) * 10].length;
                            right = right.substring(replace_end,input_word.length); 
                            console.log("left: " + left);
                            console.log("right: " + right);
                            input_word = left + select_letter[keyin - 49 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                        }
                        $("#show").html("");    
                        $("#show_flat").html("");
                    ...
                    if (mode == 1){
                        if (keyin == 48 || keyin == 96){
                            input_word = left + select_letter[9 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                            word_length = select_letter[9 + (thispage - 1) * 10].length;
                            prefix_key = select_letter[9 + (thispage - 1) * 10];
                        }
                        else if (keyin >= 97){
                            input_word = left + select_letter[keyin - 97 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                            word_length = select_letter[keyin - 97 + (thispage - 1) * 10].length;
                            prefix_key = select_letter[keyin - 97 + (thispage - 1) * 10];
                        }
                        else if (keyin >= 49){
                            input_word = left + select_letter[keyin - 49 + (thispage - 1) * 10] + right;
                            textbox.html(input_word);
                            word_length = select_letter[keyin - 49 + (thispage - 1) * 10].length;
                            prefix_key = select_letter[keyin - 49 + (thispage - 1) * 10];
                        }
                        $("#show").html("");    
                        $("#show_flat").html("");
                        prompt_txtbox.val("選字成功!");
                    ...
                    if (mode == 1){
                        ...
                        for(var i = 0; i < auto_letter.length; i++){
                        	...
                        	if (i == 0){
                        		index = "first";
    	                		three_word_loc[i] = auto_start;
                        		three_pinyin_loc[i] = auto_start;
                        		three_word_loc[i + 1] = 999999;
                        		three_pinyin_loc[i + 1] = 999999;
                        	}
                        	else if (i == 1){
                        		index = "second";
                        		three_word_loc[i] = auto_start + auto_letter[i - 1].length;
                        		three_pinyin_loc[i] = auto_start + auto_letter[i - 1].length;
                        		three_word_loc[i + 1] = 999999;
                        		three_pinyin_loc[i + 1] = 999999;
                        	}
                        	else if (i == 2){
                        		index = "third";
                        		three_word_loc[i] = three_word_loc[i - 1] + auto_letter[i - 1].length;
                        		three_pinyin_loc[i] = three_word_loc[i - 1] + auto_letter[i - 1].length;
                        	}
                        ...
                        //input_loc = auto_start + characters_count;	// 調整input_loc
                        input_loc++;
            ...
            /*function add_key_underline(key){
            	var textbox = $("#input");
            	var text = textbox.html();
            	console.log("(ADD)text: " + text);
            	var index = text.indexOf(search_key);
            	if (index > 0){
            		var span_tag = [
            		...
            		var right = text.substring(index + search_key.length,text.length);      
            		text = left + '<span id="temp_pinyin" class="in_pinyin_window">' + key + '</span>' + right;		
            	}
            	else if (index == 0){
            		var right = text.substring(search_key.length,text.length);
            		text = '<span id="temp_pinyin" class="in_pinyin_window">' + key + '</span>' + right;
            	}
            	textbox.html(text);
            	var caret_pos = input_loc + search_key.length;
            	textbox.focus().setCursorPosition(caret_pos);
            }*/
            ...
            function deleteWord(keyCode){                                           // 處理選字後的刪除事件，不包含search_key的增減
                ...
                if (keyCode >= 65 && keyCode <= 90){                                // 反白並且用輸入英文來取代時的情形
                    if (textbox.html().length <= input_len){
                        var char_arr = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s",          
                         "t", "u", "v", "w", "x", "y", "z"];
                        var text = textbox.html();
                        text = remove_tags(text);
                        input_word = text;
            ...
            function remove_tags(text){
            	if (text.indexOf("</span>") >= 0){									    // 如果是智慧選詞模式下，會出現span
                	if (text.indexOf('<span id="first" class="in_pinyin_window">') >= 0){
	                	text = text.replace('<span id="first" class="in_pinyin_window">',"");		// 不取html tag
	                	text = text.replace('</span>',"");								// 得到真正的value
                	}
	                if (text.indexOf('<span id="second" class="in_pinyin_window">') >= 0){
	                	text = text.replace('<span id="second" class="in_pinyin_window">',"");		// 不取html tag
	                	text = text.replace('</span>',"");								// 得到真正的value
	                }
	                if (text.indexOf('<span id="third" class="in_pinyin_window">') >= 0){
	                	text = text.replace('<span id="third" class="in_pinyin_window">',"");		// 不取html tag
	                	text = text.replace('</span>',"");								// 得到真正的value
	                }
                }
                if (text.indexOf("</u>") >= 0) {
            ...
            function getKey(keyCode){                                               // 產生要搜索mysql的key值，並考慮增修情形
                var textbox = $("#input");
                var DOM_textbox = document.getElementById("input");
                var now_loc = getCaretCharacterOffsetWithin(DOM_textbox);
                var text = textbox.html();
                console.log("getKey中的html: " + text);
                text = remove_tags(text);
                var temp_key_len = text.length - input_len;
                console.log("移除tag後的text: " + text);
                ...
                if (keyCode >= 65 && keyCode <= 90 || keyCode == 32){ 
                    if (input_len == 0){                                            // 當是打第一個字時
                        ...
                        if (input_loc == 0){                                        // 拼音插入在最前方
                            ...
                            var left = key_part.substring(0,search_key_loc);
                            var right = key_part.substring(search_key_loc,temp_key_len);
                            console.log("(getKey)left: " + left);
                            console.log("(getKey)right: " + right);
                            search_key = left + right;                       
                            search_key = search_key.replace("&nbsp;"," ");  
                        }      
                        //search_key = remove_tags(search_key);
                    }
                }
            ...
            function get_sel_mode(text){                                    // 判斷dropdown中的值，並決定輸入法模式
                ...
                if (text == "英數模式"){
                    ...
                    prompt_flat_txtbox.val("已切換至英數模式!");
                }
                textbox.focus();
                caption_effect();   
            }
            ...
            function contructer_undo_record(index,mode,len,duplicate_val,word,loc,key,a_key,a_start,a_end,a_len){       // undo_record 物件的建構子
                this.index = index;
                this.mode = mode;
                this.len = len;
    ...
    <body>
            ...
            <div class="container">
                ...
                <div id="navbarCollapse" class="collapse navbar-collapse">
                    ...
                    <ul class="nav navbar-nav navbar-right">
                        ...
                        <!--<li><a href="#login" id="nav_login" data-toggle="modal"><span class="glyphicon glyphicon-user"></span> 會員登入</a></li>-->
                        <?php
                            $url = "./auth/login";
                            $exp = "會員登入/註冊";
                            if (Auth::user())
                            ...
                            $string = "<li><a href=". $url. " id=\"nav_login\" data-toggle=\"modal\"><span class=\"glyphicon glyphicon-user\"></span> ". $exp. "</a></li>";
                            echo $string;
                        ?>   
                        <!--@if (Auth::user())
        ...
        <div class="container">
            <div class="row" style="text-align: center">
                ...
                <div class="col-xs-12 col-sm-4 col-md-4" id="up_right">
                    <div class="login_span pull-right">
                        <br>
                        <p id="login_status">
                            您當前的身分是:
                            <span class="login_right_span label label-default" id="login_person">
                                <?php if (Auth::user()) echo Auth::user()->name; else echo "訪客";?>
                            </span>
                        </p>
                    </div>
                </div>
                ...
                <div class="col-xs-12 col-sm-4 col-md-4" id="left" style="text-align: center">
                    ...
                    <div class="jumbotron" id="public_note">
                        <div style="margin-top: -15px">
                            <h3>系統公告</h3>
                            <p style="text-align: right"><small>3/25.2015</small></p>
                        ...
                        <div class="hidden-xs">
                            <div class="ui left pointing dropdown icon button" tabindex="0" id="select_mode" data-variation="large">
                                ...
                                <div class="menu">
                                    ...
                                    <div class="item" style="font-size: 10px" onclick="get_sel_mode('英數模式')" onkeydown="enter_sel_mode(event,'英數模式')"><i class="pencil icon"></i>英數模式</div>
                                </div>
                            </div>
                            <!--<span>組字區域: </span><span id="pinyin_window"></span>-->
                            <br>
                        ...
                        <div class="visible-xs">
                            <div class="ui left pointing dropdown icon button" id="select_mode_flat" data-offset="-8" style="margin-top: 10px; background-color: #FF9494; color: #FFF; font-weight: bold; font-family: '微軟正黑體'; font-size: 9px">
                                ...
                                <div class="menu">
                                    ...
                                    <div class="item" style="font-size: 10px" onclick="get_sel_mode('英數模式')"><i class="pencil icon"></i>英數模式</div>
                                </div>
                            </div>
        ...
        <div id="message_board" class="modal fade" tabindex="-1" data-width="400" style="display: none;">
            <div class = "modal-header">
                ...
                <h4 style="margin-left: 18px">聯絡我們</h4>
            </div>
                ...
                <div class="form-group" id="info_form_msg">
                    <div class="ui fluid input" class="form-control" style="font-size: 14px; font-weight: bold">
                        ...
                        <div style="margin-top: 8px">
                            ...
                            <input type="text" class="form-control" id="mes_email">
                        </div>
                        <div style="margin-top: 8px">
                            <p><i class="key icon"></i>內容</p>
                            <textarea id="comment" name="comment" aria-required="true" style="height: 220px; width: 100%"></textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script src="{{ asset('/jQuery-TE_v.1.4.0/jquery-te-1.4.0.js') }}"></script>
        <script src="{{ asset('/js/jquery.zclip.js') }}"></script>
        <script src="{{ asset('/js/buttons.js') }}"></script>
        <script src="{{ asset('/js/jsnow.js') }}"></script>
